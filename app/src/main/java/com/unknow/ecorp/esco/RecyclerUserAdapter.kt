...
class RecyclerUserAdapter(private val jsonArray: JsonArray) : RecyclerView.Adapter<RecyclerUserAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val idPlusUsername = jsonArray[position].asJsonObject["id"].asString + " - " + jsonArray[position].asJsonObject["nombre"].asString
        holder.username.text = idPlusUsername
        holder.email.text = jsonArray[position].asJsonObject["330"].asString
    }
